...
const ViewMatchRacing = () => {
  ...
  const handleBackOpen = (data, isCombined = false) => {
    ...
    if (isCombined) {
      ...
      const selectedRunners = matchOddsData.runners.filter((_, index) =>
        matchOddsSelected.includes(index + 1),
      );
      const selectionIds = selectedRunners.map((runner) => runner.selectionId);

      const teamNames = selectedRunners
        .map((runner) => runner.selectionName)
        .join("+");
            ...
            <div className="xl:block hidden">
              {inplayMatch && inplayMatch?.matchName ? (
                <div className="bg-[var(--secondary)] tem-center px-2 py-1.5 flex justify-between">
                  <span className="text-black text-[14px] font-semibold flex justify-start items-center">
                    <IoHome /> <FaAngleRight /> {inplayMatch?.sportType}{" "}
                    <FaAngleRight /> {inplayMatch?.matchName}
                  </span>
                  <div
                    onClick={() => handelScoreModalComplete()}
              ...
              <div className="bg-[var(--primary)] flex justify-between px-2">
                <div className="text-xs font-light text-white flex items-center py-1">
                  {inplayMatch?.matchName} / {inplayMatch?.marketName || ""}
                </div>
              </div>
            </div>
            ...
            <div className="xl:hidden block">
              {inplayMatch && inplayMatch?.matchName ? (
                <div className="bg-[var(--secondary)] item-center px-2 py-1.5 flex justify-start">
                  <div
                    className="text-black text-[14px] font-semibold flex justify-start items-center truncate w-1/2"
                    onClick={closeModal}
                  >
                  ...
                  <div className="flex justify-between w-full">
                    <div
                      className="text-start px-1 text-black text-[14px] font-semibold flex justify-start items-center"
                      onClick={() => handleBets() || handelScoreModalComplete()}
                    >
                      MY BETS
                    </div>
                    <div
                ...
                <ul class="flex-wrap space-x-2 rtl:space-x-reverse flex rounded-t-[5px] w-full bg-[var(--secondary)] text-center">
                  <li role="presentation" class="group">
                    <button
                      type="button"
                      role="tab"
                      class={`inline-block text-center text-[10px] font-bold uppercase w-[130px]  h-[30px]  rounded ${selectedType === "score" ? "bg-[var(--primary)] text-white border-b-2" : ""}`}
                      onClick={() => handleWatchButtonClick("score")}
                    >
                  ...
                  <li role="presentation" class="group">
                    <button
                      type="button"
                      role="tab"
                      class={`inline-block text-center text-[10px] font-bold uppercase w-[130px]  h-[30px]  rounded ${selectedType === "tv" ? "bg-[var(--primary)] text-white border-b-2" : ""}`}
                    >
                      <span slot="title" class="flex gap-2">
                        <span
                          class="m-auto flex gap-1 items-center"
                          onClick={() => handleWatchButtonClick("tv")}
                        >
                          Live TV{" "}
                        ...
                        handleBackOpen(
                          {
                            ...
                            oddsType: "Match Odds",
                            betfairMarketId:
                              finalSocket["Match Odds"]?.marketId,
                          },
                          true,
                        );
              ...
              <div className=" flex justify-center items-start z-50">
                <div className="bg-white w-full max-w-3xl rounded-md shadow-lg md:m-4 m-1 p-0">
                  ...
                  <div className="flex justify-between items-center border-x border-t border-[#C6D2D8] bg-white w-full">
                    {["oddsBetData", "UnsettleBets", "fancyBetData"]?.map(
                      (tab) => (
                        <button
                          key={tab}
                        ...
                        <div className="overflow-hidden w-full ">
                          <table className="min-w-full capitalize border border-[#f8f8f8]">
                            ...
                            <tbody>
                              ...
                              {/* Unsettle Bets */}
                              <div className="bg-[var(--primary)] flex justify-between px-2">
                                <div className="text-xs  font-light text-white md:flex hidden items-center py-1">
                                  {inplayMatch?.matchName}
                                </div>
                              </div>{" "}
